		...
		for (Field field : fields)
		{
			if (field.getAnnotation(ContextVariable.class) != null && context.containsKey(field.getName()))
			{
				...
				else
				{
					...
					throw new RuntimeException(String.format("Unsuported type. Cannot pass value to variable %s of step %s. Variable type is unsuported.", field.getName(), step.getClass().getName()));
				}
			}
		}
		...
		for (Field field : fields)
		{
			...
			if (contextVar != null)
			{
				...
				try
				{
					...
					{
						//TODO: Add support for arrays
						throw new RuntimeException(String.format("Unsuported type. Cannot pass value to variable %s of step %s. Variable type is unsuported.", field.getName(), step.getClass()
								.getName()));
					}
				}
